...
- **Auto screenshots:** Add `--auto-screenshot` to capture a PNG after every browser step. Images land in `.doc-detective/run-<runId>/screenshots/` at stable paths for run-over-run comparison.
- **Per-run artifacts:** Results are archived by default at `<output>/.doc-detective/run-<runId>/testResults.json`, alongside any screenshots from that run.
